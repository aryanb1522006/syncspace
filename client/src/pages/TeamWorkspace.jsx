...
function TeamRail({ team, role }) {
  ...
  return <>
    ...
    <Button to={role === 'owner' ? `/projects/${team.projectId ?? team.project_id}/applications` : '/dashboard'}>{role === 'owner' ? 'Review applications' : 'Discover projects'}</Button>
  </>;
}
...
export function TeamWorkspace() {
  ...
  return <AppShell className="workspace-shell" rightRail={<TeamRail team={team} role={user?.role} />}>
    ...
    <div className="members-strip">{team.members.map((member, index) => <div className="member" key={member.id}><span className={`avatar ${index ? 'avatar--lavender' : 'avatar--mint'}`}>{member.initials ?? member.name.split(' ').map((part) => part[0]).join('')}</span><span><strong>{member.name}</strong><small>{member.role_label || 'Team member'}</small></span></div>)}{openSpots > 0 && <div className="member member--open"><span><UserRound /></span><strong>{openSpots} {openSpots === 1 ? 'spot' : 'spots'} open</strong></div>}</div>
