...
function createCompatibilityScene(canvas, labels, reason = "webgl-unavailable") {
  ...
  if (!shell) return { setScrollProgress() {}, dispose() {} };
  ...
  return {
    ...
    dispose() {
      canvas.hidden = false;
      shell.classList.remove("constellation-shell--fallback");
      shell.style.removeProperty("--fallback-scroll");
      delete shell.dataset.renderer;
      delete shell.dataset.rendererReason;
      Object.values(labels).forEach((label) => {
        if (label) delete label.dataset.fallbackNode;
      });
    },
  };
}
...
export function createConstellationScene(canvas, labels) {
  if (!canvas) return { setScrollProgress() {}, dispose() {} };
  ...
  return {
    ...
    dispose() {
      ...
      if (rendererAvailable) renderer.dispose();
