...
export function ProjectDetail() {
  ...
  useEffect(() => {
    ...
    const load = user?.role === 'student'
      ? Promise.all([api.getProject(id), api.listApplications(), api.recommendations().catch(() => ({ recommendations: [] }))])
      : Promise.all([api.getProject(id), Promise.resolve({ applications: [] }), Promise.resolve({ recommendations: [] })]);
  ...
  const isOwner = user?.role === 'owner' && Number(ownerId) === Number(user.id);

  return <AppShell>
    <Link className="back-link app-back" to="/dashboard"><ArrowLeft />{user?.role === 'owner' ? 'My projects' : 'Discover'}</Link>
    ...
    <div className="project-detail__layout"><div className="project-detail__main"><section><h2>What the team is building</h2><p>{project.longDescription ?? project.description}</p></section><section><h2>Skills this project needs</h2><div className="skill-requirements">{project.skills.map((skill) => <div key={skill.id}><span><Check />{skill.name}</span><small>{skill.importance}</small></div>)}</div></section>{project.match && <MatchBreakdown match={project.match} domain={project.domain} />}</div>
      <aside className="project-detail__aside"><div className="project-icon"><Leaf /></div><dl><div><dt><UsersRound />Team</dt><dd>{memberCount} of {teamSize} spots filled</dd></div><div><dt><Clock3 />Commitment</dt><dd>{commitment} hours per week</dd></div><div><dt><CalendarDays />Deadline</dt><dd>{new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium' }).format(new Date(project.deadline))}</dd></div></dl><p>Led by <strong>{project.owner_name}</strong></p>
        {isOwner ? <Button to={`/projects/${id}/applications`}>Review applications</Button> : user?.role === 'owner' ? <p className="decision-note">This project is managed by another owner.</p> : application?.status === 'accepted' && application.teamId ? <Button to={`/team/${application.teamId}`}>Open team workspace</Button> : application ? <Button to="/applications" variant="secondary">Application {application.status}</Button> : <Button onClick={apply} disabled={busy}>{busy ? 'Sending…' : 'Apply to join'}</Button>}
